...
import { isAuthorized, server } from '../pages/api/service';
...
function DashboardContent({ userData, children, updateUser }: IDashboardContentProps) {
    const [open, setOpen] = React.useState(true);
    const [withdrawModal, setWithdrawModal] = React.useState<boolean>(false);
    const toggleDrawer = () => {
        setOpen(!open);
    };
    const { name, userBalance } = userData;
    return (
...
export default function Dashboard({ children }: IDashboardProps) {
    const [userData, setUserData] = React.useState<ITokenInfo>();
    React.useEffect(() => {
    ...
    return (
        <>
            {userData ? (
                <DashboardContent updateUser={updateUser} userData={userData}>
                    {children}
                </DashboardContent>
            ) : (
                <LinearProgress />
